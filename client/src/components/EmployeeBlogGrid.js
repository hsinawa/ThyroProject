...
import { GetBlogByEmployeeAction } from '../actions/BlogAction'
import {BlogsByEmployeeReducer} from '../reducers/BlogReducers'
...
const EmployeeBlogGrid = ({id}) => {
    const dispatch = useDispatch()

    useEffect(()=>{
        dispatch(GetBlogByEmployeeAction({id}))
    },[dispatch])

    const { loading, Blogdata, error } = useSelector(
        (state) => state.BlogsByEmployeeReducer
      );
...
export default EmployeeBlogGrid